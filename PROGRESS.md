...
| T4 — Overlay visual check of buffered masks | 🔍 Awaiting owner review | 2026-07-07; `src/dataset_tools/overlay_check.py` (TDD waived by owner for this script). Contact sheet at `data/overlay_check/overlay_check.html`: same 6 sampled images (seed 42) × three buffer radii (2.0 / 3.0 / 4.5 px), full overlay + 3× mid-curve zoom per cell, sources from `data/images/`. Owner decides the buffer radius. Source figures resolved from the `datasheet-studio-v2` cache (160/164 covered; 4 batch images have no matching local figure crop — different OCR-run numbering in backup caches). |
