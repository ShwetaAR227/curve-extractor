...
| T30 — zth_vs_time classical extractor (TDD, legacy port) | ✅ Done (pending owner review) — **40/40 tests green, suite 1099 passing, 0 new regressions** | 2026-07-23; `src/extraction/classical_zth.py`, ported from legacy `D:\Extractor\5_opencv_extract\zth_extract.py` per CLAUDE.md §6/§2, owner-approved design (does NOT reuse the shared `process_detections`/`curve_detection.py` core rdson/vgsth use — zth needs its own tick parsing, axis auto-fit, clustering, tracer, single-pulse picker, Foster-table OCR reader, and physics fit; only the final `schema.build_result` step is shared). **Ported unchanged**: unit-aware tick parsing (ns/us/ms/s, SI/exponent/superscript/mojibake forms), `fit_axis_auto` (linear-vs-log auto-pick), `derive_calibration_zth`/`pixel_to_data` (zth's own calibration dict, deliberately NOT `src.calibration.ticks.pixel_to_data` — no runtime collision, documented in the docstring), `cluster_into_curves_zth` (no top-N truncation), `trace_curve` (bottom-most/95th-percentile per column), `pick_single_pulse` (weighted edge/flat/OCR picker v2), `parse_foster_table_from_ocr` + `_try_parse_value` (printed-table reader), `fit_foster` (1-element Foster RC fit, optional fixed-R constraint), `find_rth_jc_in_full_extraction`/`pick_rth_constraint`. **Deliberate restructuring** (same math, not a logic change, flagged to owner beforehand): the inline threshold/gridline-removal/close block inside legacy's `process()` extracted into a named `_clean_for_clustering()` helper purely for testability. **New code**: `run_classical_pipeline()` — printed-table-first (confidence 1.0, `rc_pairs`/`rth_jc_steady_state` as bonus curve fields, skips the CV path entirely), else digitize→calibration-disaster gate→scale-ratio/skip_fit gate→Foster fit→r²<0.5 gate, remapping legacy's many status strings onto the two-status system (one **deliberate non-literal remap**: legacy's "skip_fit" — Rth constraint disagrees with the observed curve, legacy calls this "clean" despite warning "curve trace unreliable" in the same breath — mapped to `needs_review` here, keeping legacy's exact warning text as `review_reason`); `_read_full_extraction_for_rth()` — a NEW, owner-approved self-contained read of `<stage3_root>/<device>/full_extraction.json` for the printed Rth_JC table only (optional `stage3_root` kwarg, else `LINEFORMER_STAGE3_ROOT` env var — same convention as `stage3_loader.py`), chosen specifically to avoid touching `live_stages.py`'s fixed 5-kwarg classical-pipeline call site; any read failure degrades to "no constraint" (logged, never a crash) since an unconstrained Foster fit is a normal legacy outcome. Curve name fixed to `"single_pulse"` (matches the already-collected CVAT training data), units fixed to `"K/W"`, confidence = r² clamped to [0, 1] (1.0 for a table read, 0.0 for a total fit failure). **Legacy figure-auto-classifier (`score_zth`/`auto_pick_zth_figure`) deliberately NOT migrated** — duplicates `curve_registry.py`'s job; a `zth_vs_time` `CurveTypeSpec` entry is a separate, smaller follow-up task, not started. `extraction_registry.py` has no `zth_vs_time` entry yet either — also a separate follow-up, not done here (no frozen file touched). **TDD**: `tests/test_classical_zth.py` (40 tests) written and confirmed red (`ModuleNotFoundError`) before any implementation. First green run found **one real implementation bug**: `_TAU_HEADER_RE` required a literal ASCII `t` before its OCR-variant char class, so it could never match a genuine Greek τ ("τi (sec)") — fixed by allowing `[tτ]` as the leading character (affected 6 of the 8 first-pass failures: the printed-table path never fired, table parsing itself, and confidence-clamping). Also found, exactly as anticipated during red-phase review, **two test-fixture bugs** (not implementation bugs) — fixed in the fixtures, never by weakening assertions: (1) the calibration-disaster test's monkeypatched trace used decreasing pixel-row values, which on this chart's log-y pixel mapping represents an *increasing* data curve (rise_ratio > 1), the opposite of what the test needed to trigger `rise_ratio < 1.0` — changed to increasing pixel-row values; (2) the skip_fit test's forced `rth_constraint=5.0` was close enough to the fixture's real drawn asymptote (r=2.0) that `scale_ratio≈0.4` stayed inside the allowed `[0.05, 3.0]` band and never triggered skip_fit — changed to `50.0` (`scale_ratio≈0.04`, outside the band). **Full suite: 1099 passing** (was 1059 before this task), zero new regressions — 2 failures remain, both pre-existing and unrelated (`test_live_stages.py::test_model_pipeline_called_with_expected_arguments`, `test_ticks_dual_side.py::test_if_vs_vsd_existing_real_device_tests_do_not_regress`; confirmed via `git show 7354c14 --stat` to originate from an AWS/Linux-authored commit hitting Windows for the first time — `NotADirectoryError: [WinError 267]` is a Windows-only error class, so this is a first-time platform surface, not a regression from this task). No frozen file modified — only the new `classical_zth.py` and its own test file. |
